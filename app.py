...
    #lowest_vol = df["volume"].min() Chat gpt revision
    #is_lowest = last["volume"] <= lowest_vol Chat gpt revision

    # Only consider candles till CURRENT candle (prevents future bias)
     df_till_now = df.iloc[:len(df)]

     lowest_vol = df_till_now["volume"].min()

    # STRICT match only (no duplicates)
...
    # Sure shot only if previous signal exists AND not executed
       is_sure_shot = (
       symbol in PENDING_SIGNALS and
       symbol not in ACTIVE_TRADES
)
...
                    # Allow upgrade ONLY if new signal is stronger (lower volume)
                      if symbol in PENDING_SIGNALS:
                      old_signal = PENDING_SIGNALS[symbol]

                   # Upgrade only if volume is strictly lower
                      if signal["risk"] <= old_signal["risk"]:
