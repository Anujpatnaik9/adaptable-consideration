...
FINAL V6.2 FULL BOT (SECTOR + SCANNER + UPGRADE + SAFETY)

COPY PASTE AND DEPLOY

==============================================================

import os, time, requests import pandas as pd from datetime import datetime import pytz from kiteconnect import KiteConnect import threading

================= CONFIG =================

API_KEY = os.getenv("KITE_API_KEY") ACCESS_TOKEN = os.getenv("KITE_ACCESS_TOKEN") TELEGRAM_TOKEN = os.getenv("TELEGRAM_TOKEN") CHAT_ID = os.getenv("TELEGRAM_CHAT_ID")

TOTAL_CAPITAL = 500000 RISK_PER_TRADE = 5000 MAX_TRADES = 2 EXIT_TIME = "15:15" IST = pytz.timezone("Asia/Kolkata")

kite = KiteConnect(api_key=API_KEY) kite.set_access_token(ACCESS_TOKEN)

================= GLOBALS =================

TRADES_TODAY = 0 ACTIVE_TRADES = {} PENDING_ORDERS = {} LAST_UPDATE_ID = None DIRECTION = None SELECTED_SECTORS = set()
...
def send_telegram(msg): url = f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/sendMessage" requests.post(url, data={"chat_id": CHAT_ID, "text": msg})

def read_telegram(): global LAST_UPDATE_ID, DIRECTION, SELECTED_SECTORS
...
def get_data(symbol): now = datetime.now(IST) data = kite.historical_data( kite.ltp(f"NSE:{symbol}")[f"NSE:{symbol}"]["instrument_token"], now.replace(hour=9, minute=15), now, "5minute" ) df = pd.DataFrame(data) if len(df) < 4: return None return df

================= SIGNAL =================

def check_signal(df): if df is None or len(df) < 4: return None
...
def place_entry(symbol, side, entry, qty): return kite.place_order( variety=kite.VARIETY_REGULAR, exchange=kite.EXCHANGE_NSE, tradingsymbol=symbol, transaction_type=kite.TRANSACTION_TYPE_BUY if side=="LONG" else kite.TRANSACTION_TYPE_SELL, quantity=qty, order_type=kite.ORDER_TYPE_SL, price=round(entry,1), trigger_price=round(entry,1), product=kite.PRODUCT_MIS )

def place_sl(symbol, side, sl, qty): return kite.place_order( variety=kite.VARIETY_REGULAR, exchange=kite.EXCHANGE_NSE, tradingsymbol=symbol, transaction_type=kite.TRANSACTION_TYPE_SELL if side=="LONG" else kite.TRANSACTION_TYPE_BUY, quantity=qty, order_type=kite.ORDER_TYPE_SLM, trigger_price=round(sl,1), product=kite.PRODUCT_MIS )

def cancel(order_id): try: kite.cancel_order(variety=kite.VARIETY_REGULAR, order_id=order_id) except: pass

================= TRADE =================

def execute_trade(symbol): global TRADES_TODAY
...
def monitor(): global TRADES_TODAY
...
def scanner(): while True: try: read_telegram()
...
threading.Thread(target=monitor).start() scanner()
